...
int main(int argc, char **argv) {
   ...
   for(int i=0; i<runs; i++){      
      if(do_flush){
         flush_buf(output_buffers[i%num_bufs], compressed_sizes[i%num_bufs]);
      ...
      // printf("latency,%ld\n", end-start);
      int mismatch;
      if( (mismatch = compare_buffers(output_buffers_2[i%num_bufs], input_buffers[i%num_bufs], size)) != -1){
   ...
   for(int i=0; i<runs; i++){
      if(do_flush){
         ...
         flush_buf(input_buffers[i%num_bufs], size);
      }
      ...
      uint64_t end = micros();
      // printf("latency,%ld\n", end-start);
      if(compare_buffers(output_buffers[i%num_bufs], backup_buffers[i%num_bufs], stream.total_out) != -1){
