	...
	with open('result.csv', 'w') as f:
		...
		for test in TESTS:
			...
			for target in TARGETS:
				...
				if (os.stat(test).st_size == size):
					r = subprocess.check_output( [ "sudo", "QZ_ROOT=QATzip;", "-E", "taskset", "-c", "0", "./QATzip/test/harness", "-m", "4", "-l", str(runs), "-t", "1", "-D", "decomp", "-P", "busy", "-i",  test]).decode("utf-8")[:-1]
