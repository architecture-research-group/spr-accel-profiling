...
int main(int argc, char **argv) {
   ...
   int status = LZ4_decompress_safe((char *)input_buffer, (char *)output_buffer, compressed_size, outlen);
   ...
   for (int j = 0; j < runs; j++) {
      {
         ...
         for (int i = 0; i < iterations_per_run; i++) {
            ...
            size = LZ4_decompress_safe((char *)input_buffer, (char *)output_buffer, compressed_size, outlen);
         }
         uint64_t end = nano();
         double bb = (end-start);
...
//    printf("%s-decomp(ns),median:%lf,mean:%lf,max:%lf,min:%lf\n", argv[1],times[runs/2], mean, times[runs-2], times[0]);
   printf("%lf,%lf,%lf,%lf,%lf\n",1.0 * size, times[runs/2], mean, times[runs-2], times[0]);
